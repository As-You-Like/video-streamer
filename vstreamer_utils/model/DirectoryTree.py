...
class DirectoryTree:
    ...
    def read_info(self):
        ...
        with properties_file.open("rb") as properties_file:
            ...
            vstreamer_utils.log_info("Read AdditionalEntryProperties from '%s'" % str(properties_file))

    def store_info(self):
        properties_file = self.directory_root / "AdditionalProperties.data"
        with properties_file.open("wb") as file:
            pickle.dump(self.additional_properties, file)
            vstreamer_utils.log_info("Stored AdditionalEntryProperties to '%s'" % str(properties_file))
