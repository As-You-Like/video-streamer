...
def main():
    if len(sys.argv) != 5:
        print("%s: invalid number of arguments (%d/%d)" % (sys.argv[0], len(sys.argv) - 1, 4), file=sys.stderr)
        return 1
    file = sys.argv[1]
    title = sys.argv[2]
    ...
    if session_bus.request_name(vstreamer_utils.DBUS_NAME) == dbus.bus.REQUEST_NAME_REPLY_PRIMARY_OWNER:
